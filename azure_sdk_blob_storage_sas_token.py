...
# Variable definition
accountname = '{azure_blob_storage_accountname}' 
accountkey = '{azure_blob_storage_accountkey}'
container_name = '{azure_blob_storage_container_name}'
timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
dataformat = 'json'
filename = 'testdata'
path = 'C:\Projekte\Temp_Projekte'
token = "{azure_blob_storage_sas_token}"
connectionstring = '{azure_blob_storage_connectionString}'

try:

    # Create the BlockBlockService that the system uses to call the Blob service for the storage account.
    block_blob_service = BlockBlobService(account_name = accountname, account_key = None, sas_token = token)
    #block_blob_service = BlockBlobService(connection_string = connectionstring)
